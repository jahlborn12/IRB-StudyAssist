...
public class Board : MonoBehaviour 
{
	...
	void Start () 
	{
		levelNum = Score.levelnum;
		GridHeight = 6+ Mathf.FloorToInt (levelNum / 4);
		GridWidth = 6 + Mathf.FloorToInt (levelNum / 4);
		...
		gameObject.transform.position = new Vector3(-.5f*GridWidth+.5f,-8f+GridHeight,0);
	...
	void Update () 
	{
		if(isMatched)
		{
			...
			{
				if(gems[i].isMatched) //Check if matched, and reset
				{
					gems[i].CreateGem();
		...
		else if(!DetermineBoardState())
		{
			...
			if(!DoesBoardContainMatches()) //Lose condition
			{
				//isMatched = true;
				//for(int i=0;i<gems.Count;i++)
				//{
	...
	{
		if (GameObject.FindWithTag ("Q") != null) 
		{
			return true;
		}
		for(int i=0;i<gems.Count;i++)
		{
			if(gems[i].transform.localPosition.y >4.6)
			{
				return true;
			}
			if(gems[i].rigidbody.velocity.y > 0.1f)
			{
				return true;
			}
